...
The proposed framework consists of three key modules: packing policy learning,
structural stability validation, and safe repacking planning. Structural
stability is evaluated using a feasibility map, which identifies unstable item
placements before they are accepted. This validation process is integrated into
both the packing policy and the repacking planner, ensuring that stability
constraints are enforced throughout the packing and repacking process.

The packing policy follows an actor-critic architecture. The actor selects a
loading position from candidate placements generated by the placement module,
while the critic estimates the expected bin utilization, denoted as `v_hat`.
When no stable placement is available for the incoming item, the safe repacking
planning module searches for a sequence of unpack, repack, and pack actions on
already packed items to create feasible space.
